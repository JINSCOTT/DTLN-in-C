	...
	if (axes == NULL) {
		...
		while (1) {
			...
			if (*cur < 0) {
				...
				*(int64_t*)get_item_array(new_dims, *cur) = -1;
			}
...
int set_matmul_shape(struct tensor* a, struct tensor* b, struct tensor* c) {
	...
	a_dim = create_array_from_array(a->dimension, a->dimension_size, sizeof(int64_t));
	b_dim = create_array_from_array(b->dimension, b->dimension_size, sizeof(int64_t));
	c_dim = create_array(sizeof(int64_t));
	...
	while (a_dim->size > b_dim->size) {
		pushfront_array(b_dim, &insert_val);
	}

	for (i = 0; i < a_dim->size - 2; i++) {
		if (*(int64_t*)get_item_array(a_dim, i) == *(int64_t*)get_item_array(b_dim, i))pushback_array(c_dim, get_item_array(a_dim, i));
		else {
			if (*(int64_t*)get_item_array(a_dim, i) == 1)pushback_array(c_dim, (int64_t*)get_item_array(b_dim, i));
			else if (*(int64_t*)get_item_array(b_dim, i) == 1) pushback_array(c_dim, (int64_t*)get_item_array(a_dim, i));
			else {
...
int set_slice_shape(struct tensor* data, int64_t* starts, int64_t* ends, int64_t* axes, int64_t* steps, struct tensor* output) {
	...
	ouput_dim = create_array(sizeof(int64_t));
	if (ouput_dim == NULL) {
		error = OPS_ALLOCATION_FAIL;
		goto cleanup;
	}
...
int set_gemm_shape(struct tensor* A, struct tensor* B, int64_t* transA, int64_t* transB, struct tensor* Y) {
	...
	pushback_array(y_dim, get_item_array(a_dim, 0));
...
int set_concat_shape(int64_t* axis, struct list* inputs, struct tensor* concat_result) {
	...
	rank = ((struct tensor*)get_data_list(inputs, 0))->dimension_size;
	...
	if (*axis < 0) axis += rank;
	y_dim = create_array_from_array(((struct tensor*)get_data_list(inputs, 0))->dimension, ((struct tensor*)get_data_list(inputs, 0))->dimension_size, sizeof(int64_t));
	if (y_dim == NULL) {
		error = OPS_ALLOCATION_FAIL;
		goto cleanup;
	}
	for (i = 1; i < inputs->size; i++) {
		cur_tensor = get_data_list(inputs, i);
		for (j = 0; j < rank; j++) {
			if (j == *axis) {
				*(int64_t*)get_item_array(y_dim, j) += cur_tensor->dimension[j];
			}
			else {
				if (*(int64_t*)get_item_array(y_dim, j) != cur_tensor->dimension[j]) {
					printf("here\n");
					error = OPS_DIMENSION_MISMATCH;
					goto cleanup;
				}
...
int set_split_shape(int64_t axis, int64_t num_outputs, struct tensor* input, int64_t* split, struct list* outputs) {
	...
	for (i = 0; i < num_outputs; i++) {
		...
		resize_tensor(get_data_list(outputs, i), y_dim, input->dimension_size, input->type);
	}
	error = OPS_SUCCESS;
cleanup:
	safe_free(y_dim);
...
int set_pad_shape(struct tensor* data, struct tensor* pads, struct tensor* axes, struct tensor* output){
	...
	output_dim = create_array_from_array(data->dimension, data->dimension_size, sizeof(int64_t));
	if (output_dim == NULL) {
		error = OPS_ALLOCATION_FAIL;
		goto cleanup;
	}
	for (i = 0; i < axes->data_size; i++) {
		*(int64_t*)get_item_array(output_dim, ((int64_t*)axes->data)[i]) += ((int64_t*)pads->data)[i] + ((int64_t*)pads->data)[i + axes->data_size];
	}
...
int set_lstm_shape(struct tensor* x, int64_t num_direction, int64_t hidden_size, struct tensor* y, struct tensor* y_h, struct tensor* y_c) {
	...
	dims = create_array(sizeof(int64_t));
	if (dims == NULL) {
		error = OPS_ALLOCATION_FAIL;
		goto cleanup;
	}
	// Set y dim
	pushback_array(dims, &x[0]);
...
int set_reducemean_shape(struct tensor* axes, int64_t* noop_with_empty_axes, int64_t keepdims, struct tensor* data, struct tensor* reduced) {
	...
	for (i = 0; i < axes->data_size; i++) {
		num = ((int64_t*)axes)[i];
		if (keepdims) {
			*(int64_t*)get_item_array(reduced_dim, num) = 1;
		}
		else {
			*(int64_t*)get_item_array(reduced_dim, num) = 0;
		}
	}
	if (keepdims == 0) {
		for (i = 0; i < reduced_dim->size; i++) {
			if (*(int64_t*)get_item_array(reduced_dim, num) == 0) {
				delete_item_array(reduced_dim, i);
				i--;
			}
		}
	}
